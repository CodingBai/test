...
class Dina(object):
    ...
    def get_yita(self, skills):
        ...
        _aa = np.sum(self._attrs * self._attrs, axis=0)  # 每个题目考察了几个知识点，_attrs的值都是0/1
        _yita[_yita < _aa] = 0
        _yita[_yita == _aa] = 1
        return _yita
...
class MlDina(BaseEmDina):
    ...
    def solve(self):
        ...
        skills = self._get_all_skills()  # 2**k * knowledge
        yita = self.get_yita(skills)
        p_val = self.get_p(yita, self._no_slip, self._guess)
        loglik = self._loglik(p_val)  # 2**k * stu
        return skills[loglik.argmax(axis=0)]  # 返回学生的知识点认知程度矩阵
...
class EmDina(BaseEmDina):
    ...
    def __init__(self, guess_init=None, no_slip_init=None, max_iter=100, tol=1e-5, *args, **kwargs):
        ...
        # item_size--question_num
        # 如果没给初始化的s和g，初始化为都是0.3
...
class BaseMcmcDina(Dina):
    ...
    def __init__(self, thin=1, burn=3000, max_iter=10000, test_no_s=None, test_g=None,*args, **kwargs):
        super(BaseMcmcDina, self).__init__(*args, **kwargs)
        self.max_iter = max_iter * thin
        self.burn = burn
        self.thin = thin
        self.test_no_s = test_no_s
        self.test_g = test_g
...
#         print(nxt-pre)
    ...
    def _get_item_params_tran_res(self, skills, no_slip, guess):
        ...
        next_no_slip[next_no_slip <= 0.4] = 0.4 + 1e-10  # 控制失误率在（0, 0.6）
        next_no_slip[next_no_slip >= 1] = 1 - 1e-10
        next_guess = np.random.uniform(guess - 0.1, guess + 0.1)
        next_guess[next_guess <= 0] = 1e-10  # 控制猜测率在（0, 0.6）
...
class McmcDina(BaseMcmcDina):
    ...
    def mcmc(self):
        ...
        for i in range(size):
            skills = self._get_skills_tran_res(skills, no_slip, guess) # 计算学生掌握程度转移概率
            no_slip, guess = self._get_item_params_tran_res(skills, no_slip, guess)
            skills_list[i] = skills      # 记录本次迭代预测结果
            no_slip_list[i] = no_slip
            guess_list[i] = guess
        est_skills = np.mean(skills_list[self.burn::self.thin], axis=0)  # 舍弃前2500次结果，对后2500次结果求均值，得到最终估计结果
    ...
    def test_mcmc(self):
        ...
        guess, guess_list, no_slip, no_slip_list, skills, skills_list = self._get_item_params_init(size)
        for i in range(size):
            skills = self._get_skills_tran_res(skills, self.test_no_s, self.test_g)
    ...
    def _get_skills_tran_res(self, skills, no_slip, guess):
        # 被试技能参数转移结果
        next_skills = np.random.binomial(1, 0.5, skills.shape) # 概率类似于抛硬币，生成一个与skills形状相同的
