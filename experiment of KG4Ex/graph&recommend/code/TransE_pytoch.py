...
class TransE:
    ...
    def training_run(self, steps=20, batch_size=400, out_file_title=''):
      
        n_batches = int(len(self.triples) / batch_size)
      
        print("batch size: ", n_batches)
        # epoch 由steps 来决定
        epochs = int(np.ceil(steps / n_batches))
        epochs = max(1, epochs)
        print("epochs: ", epochs)
        ...
        with codecs.open(out_file_title + "loss_record.txt", "w") as f1:
                f1.write(str(self.train_loss))

        # 保存模型
        # torch.save(self.model, '/mahua/transE-master/MOOPer_triple_all_model.pth')
...
if __name__ == '__main__':
    ...
    file1 = "/mahua/transE-master/MOOPer/student_6_4_130nk_all.txt"
    # file1 = "/mahua/transE-master/MOOPer/triple_no_130.txt"
    file2 = "/mahua/transE-master/MOOPer/entity2id.txt"
    file3 = "/mahua/transE-master/MOOPer/relation2id.txt"
    # file4 = "/mahua/transE-master/MOOPer/valid.txt"
    entity_set, relation_set, triple_list = dataloader(file1, file2, file3)

    transE = TransE(entity_set, relation_set, triple_list, embedding_dim=1000, lr=0.001, margin=6.0, norm=2, C=0.25)
    transE.data_initialise()

    transE.training_run(steps=30000, batch_size=1024, out_file_title="MOOPer_triple_student_6_4_130nk_")
