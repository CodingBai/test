...
for student_id in students_id:
    ...
    for epoch in range(num_epochs):
        for inputs, targets in dataloader:
            ...
            loss = criterion(one_hot.squeeze(1), inputs.squeeze(1))
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
    ...
    result_df = pd.DataFrame(result, columns=['student', 'knowledge', 'MLKC'])
    output_file_path = f'all_k_s_MLKC/k_s_MLKC_{student_id}.csv'
